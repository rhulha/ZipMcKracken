...
// doesnt need to be mutable
fn decrypt_byte(keys: & mut[u32;3]) -> u8 {
    let temp = keys[2] | 3;
...
fn main() {
    ...
    for i in 0..12 {
        let c = data[i] ^ decrypt_byte(&mut keys);
    ...
    for i in 12..data.len() {
        let c = data[i];
        let temp = c ^ decrypt_byte(&mut keys);
    ...
    file.write_all(&data[12..data.len()]).expect("Unable to write file");
